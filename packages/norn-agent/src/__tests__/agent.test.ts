...
import { NornAgent } from '../agent'

type MockFetch = ReturnType<typeof vi.fn>

describe('NornAgent SDK', () => {
  ...
  afterEach(() => {
    vi.useRealTimers()
    vi.unstubAllGlobals()
    delete process.env['NORN_DISABLED']
  })

  it('startRun sends correct run_start payload', async () => {
    const agent = new NornAgent({ apiKey: 'test-key', host: 'http://localhost:3000' })
  ...
  it('no-op mode: zero fetch calls across the full run lifecycle', async () => {
    process.env['NORN_DISABLED'] = 'true'
